...
def load_tasks() -> List[Task]:
    ...
    try:
        ...
        print(
            "Data file is corrupted. Move/delete ~/.tiny_todo.json and retry.",
            file=sys.stderr,
        )
        sys.exit(2)
...
def add_task(text: str, priority: int = 3, due: Optional[str] = None):
    ...
    save_tasks(tasks)
    print(
        f"Added #{t.id}: {t.text} (p{t.priority}"
...
def list_tasks(view: str = "open"):
    ...
    for t in tasks:
        if t.due:
            due_fmt = datetime.strptime(t.due, "%Y-%m-%d").strftime("%d-%m-%Y")
        else:
            due_fmt = ""
        status = (
            "done"
            if t.done
                ...
                "OVERDUE"
                if (
                    t.due and datetime.strptime(t.due, "%Y-%m-%d").date() < date.today()
                )
                else "open"
            )
        )
...
def valid_date(s: str) -> str:
    ...
    try:
        ...
        raise argparse.ArgumentTypeError("Date must be DD-MM-YYYY")
...
def build_parser():
    ...
    p_add.add_argument(
        ...
        choices=range(1, 6),
        default=3,
        help="Priority 1-5 (1=high)",
    )
    p_add.add_argument("-d", "--due", type=valid_date, help="Due date DD-MM-YYYY")

    p_list = sub.add_parser("list", help="List tasks")
    g = p_list.add_mutually_exclusive_group()
    g.add_argument(
        "--all", dest="view", action="store_const", const="all", help="Show all"
    )
    g.add_argument(
        "--open",
        dest="view",
        action="store_const",
        const="open",
        help="Only open (default)",
    )
    g.add_argument(
        "--done", dest="view", action="store_const", const="done", help="Only completed"
    )
    ...
    p_test = sub.add_parser("test", help="Test commands")
